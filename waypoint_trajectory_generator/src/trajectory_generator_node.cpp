...
{ 
    ...
    */
    ROS_INFO("TIME ALLOCATE");

    VectorXd time(Path.rows() - 1);

    const double acc_dec_t = _Vel / _Acc;
    const double distance_compare = _Acc * acc_dec_t * acc_dec_t; //  1/2_Acc * acc_dec_t * acc_dec_t + 1/2_Acc * acc_dec_t * acc_dec_t 上升阶段加下降阶段

    double segment_t;

    for(int i = 1; i < Path.rows(); ++i)
    {
        double distance = (Path.row(i) - Path.row(i-1)).norm();
        if(distance > distance_compare)
            segment_t = 2 * acc_dec_t + (distance-distance_compare) / _Vel;
        else
            segment_t = std::sqrt(distance/_Acc);

        time[i-1] = segment_t;

    }

    ROS_INFO("TIME ALLOCATE DONE");
